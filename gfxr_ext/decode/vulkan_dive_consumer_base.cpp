...
PointerDecoder<uint32_t*>*                                                 ppMaxPrimitiveCounts)
{
    ...
    Decoded_VkAccelerationStructureBuildGeometryInfoKHR::struct_type* infos = pInfos ?
                                                                              pInfos->GetPointer() :
                                                                              nullptr;
    unsigned int** max_primitive_counts = ppMaxPrimitiveCounts ?
                                          ppMaxPrimitiveCounts->GetPointer() :
                                          nullptr;
    nlohmann::basic_json<nlohmann::ordered_map>&
    max_primitive_counts_json = args["ppMaxPrimitiveCounts"];

    if (infoCount > 0 && infos != nullptr && max_primitive_counts != nullptr)
    {
        for (uint32_t i = 0; i < infoCount; ++i)
        {
            FieldToJson(max_primitive_counts_json[i],
                        max_primitive_counts[i],
                        infos[i].geometryCount,
                        json_options);
        }
    }
    util::DiveFunctionData function_data("vkCmdBuildAccelerationStructuresIndirectKHR",
                                         UpdateAndGetCommandBufferRecordIndex(commandBuffer),
                                         call_info.index,
                                         args);
                                                              ...
                                                              PointerDecoder<uint8_t>* pValues)
{
    ...
    util::DiveFunctionData function_data("vkCmdPushConstants",
                                         UpdateAndGetCommandBufferRecordIndex(commandBuffer),
                                         call_info.index,
                                         args);
    WriteBlockEnd(function_data);
...
void VulkanExportDiveConsumerBase::Process_vkUpdateDescriptorSetWithTemplate(
const ApiCallInfo&               call_info,
format::HandleId                 device,
...
DescriptorUpdateTemplateDecoder* pData)
{
    ...
    util::DiveFunctionData function_data("vkCmdPushDescriptorSetWithTemplateKHR",
                                         UpdateAndGetCommandBufferRecordIndex(commandBuffer),
                                         call_info.index,
                                         args);
    WriteBlockEnd(function_data);
...
pPushDescriptorSetWithTemplateInfo)
{
    ...
    const util::JsonOptions json_options;
    auto&                   args = dive_data["args"];
    const StructPointerDecoder<Decoded_VkPushDescriptorSetWithTemplateInfo>*
    info = pPushDescriptorSetWithTemplateInfo;

    HandleToJson(args["commandBuffer"], commandBuffer, json_options);
    FieldToJson(args["pPushDescriptorSetWithTemplateInfo"], info, json_options);

    util::DiveFunctionData function_data("vkCmdPushDescriptorSetWithTemplate2KHR",
                                         UpdateAndGetCommandBufferRecordIndex(commandBuffer),
                                         call_info.index,
                                         args);
    WriteBlockEnd(function_data);
