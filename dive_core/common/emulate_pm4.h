...
public:
    ...
    bool ProcessGfxrSubmits(const std::vector<std::unique_ptr<DiveAnnotationProcessor::SubmitInfo>> &submits, const IMemoryManager &mem_manager);
    ...
    virtual bool OnCommand(uint32_t submit_index, DiveAnnotationProcessor::VulkanCommandInfo vk_cmd_info)
    {
        return true;
    }
