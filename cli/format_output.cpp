                  ...
                  std::unique_ptr<Dive::CommandHierarchy> *out_command_hierarchy)
{
    ...
    Dive::CommandHierarchyCreator        creator(*command_hierarchy, *state_tracker);
    if (!creator.CreateTrees(*command_hierarchy, *capture_data, true, std::nullopt, &log))
    {
...
int PrintTopology(const char *filename, TopologyName topology, bool verbose)
{
    ...
    Dive::CommandHierarchyCreator              creator(*command_hierarchy_ptr, *state_tracker);
    if (!creator
         .CreateTrees(*command_hierarchy_ptr, *capture_data_ptr, true, std::nullopt, &log))
    {
        std::cerr << "Error parsing capture!" << std::endl;
        return EXIT_FAILURE;
    }
