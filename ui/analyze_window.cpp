...
std::string AnalyzeDialog::GetReplayArgs()
{
    ...
    if (frame_count > 0)
    {
        ...
        return "--loop-single-frame --loop-single-frame-count " + std::to_string(frame_count);
    }

    return "--loop-single-frame";
}
                                      ...
                                      const std::string   &remote_gfxr_file)
{
    ...
    // TODO: Pass in GPU time setting
    return device_manager.RunReplayApk(remote_gfxr_file,
                                       replay_args,
                                       m_dump_pm4_box->currentIndex() == 0,
...
void AnalyzeDialog::OnReplay()
{
    ...
    // Run the pm4 replay
    if (m_dump_pm4_box->currentIndex() == 0 /*Enabled*/ ||
        m_gpu_time_box->currentIndex() == 0 /*Enabled*/ || m_enabled_settings_vector->empty())
    {
