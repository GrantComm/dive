    ...
    for (uint32_t shader_stage = 0; shader_stage < kShaderStageCount; ++shader_stage)
    {
        ...
        for (uint32_t buffer = 0; buffer < num_buffers; ++buffer)
        {
            ...
            */
            // Column 8
            // Find preferred heap
            const Dive::Pm4CaptureData          &pm4_capture_data = m_data_core.GetPm4CaptureData();
            const Dive::MemoryManager        &memory = pm4_capture_data.GetMemoryManager();
...
void BufferView::OnBufferSelectionChanged()
{
    ...
    const Dive::Pm4CaptureData   &pm4_capture_data = m_data_core.GetPm4CaptureData();
    const Dive::MemoryManager &mem_manager = pm4_capture_data.GetMemoryManager();
