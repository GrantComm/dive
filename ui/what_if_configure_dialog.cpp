...
MultiCheckComboBoxEventFilter::MultiCheckComboBoxEventFilter(QComboBox *parent)
    : QObject(parent), combo_(parent) {}

bool MultiCheckComboBoxEventFilter::eventFilter(QObject *watched, QEvent *event) {
    if (combo_ && watched == combo_->view()->viewport()) {
        if (event->type() == QEvent::MouseButtonPress) {
            QMouseEvent *mouseEvent = static_cast<QMouseEvent *>(event);
            QModelIndex index = combo_->view()->indexAt(mouseEvent->pos());

            if (index.isValid()) {
                QStandardItemModel *model =
                    qobject_cast<QStandardItemModel *>(combo_->model());
                if (model) {
            ...
            // We also need to consume the MouseButtonRelease on the checkable item
            // to be absolutely sure the combo box popup doesn't close.
            QMouseEvent *mouseEvent = static_cast<QMouseEvent *>(event);
            QModelIndex index = combo_->view()->indexAt(mouseEvent->pos());
            if (index.isValid()) {
                 const QStandardItemModel *model =
                    qobject_cast<const QStandardItemModel *>(combo_->model());
                if (model) {
                    QStandardItem *item = model->itemFromIndex(index);
                    if (item && item->isCheckable() && (item->flags() & Qt::ItemIsEnabled)) {
                        return true;
    ...
    : m_controller(controller)
{
    ...
    QStandardItem* what_if_type_placeholder = new QStandardItem("Please select a modification type");
    ...
    draw_call_filter_layout->addWidget(m_what_if_draw_call_index_count_filter_label, 0, 1, Qt::AlignRight);
    ...
    draw_call_filter_layout->addWidget(m_what_if_draw_call_vertex_count_filter_label, 0, 1, Qt::AlignRight);
    ...
    draw_call_filter_layout->addWidget(m_what_if_draw_call_instance_count_filter_label, 1, 1, Qt::AlignRight);
    ...
    draw_call_filter_layout->addWidget(m_what_if_draw_call_draw_count_filter_label, 1, 1, Qt::AlignRight);
    draw_call_filter_layout->addWidget(m_what_if_draw_call_draw_count_filter_box, 1, 2);

    m_what_if_draw_call_pso_property_filter_label = new QLabel(tr("Pipeline State Object (PSO) Property:"));
    ...
    QStandardItem* what_if_draw_call_pso_property_filter_placeholder = new QStandardItem("Select a PSO property");
    what_if_draw_call_pso_property_filter_placeholder->setFlags(what_if_draw_call_pso_property_filter_placeholder->flags() & ~Qt::ItemIsSelectable);
    m_what_if_draw_call_pso_property_filter_model->appendRow(what_if_draw_call_pso_property_filter_placeholder);
    m_what_if_draw_call_pso_property_filter_box->setModel(m_what_if_draw_call_pso_property_filter_model);
    draw_call_filter_layout->addWidget(m_what_if_draw_call_pso_property_filter_label, 2, 1, Qt::AlignRight);
    ...
    QStandardItem* what_if_render_pass_render_pass_filter_placeholder = new QStandardItem("Select a render pass");
    what_if_render_pass_render_pass_filter_placeholder->setFlags(what_if_render_pass_render_pass_filter_placeholder->flags() & ~Qt::ItemIsSelectable);
    m_what_if_draw_call_render_pass_filter_model->appendRow(what_if_render_pass_render_pass_filter_placeholder);
    m_what_if_draw_call_render_pass_filter_box->setModel(m_what_if_draw_call_render_pass_filter_model);
    draw_call_filter_layout->addWidget(m_what_if_draw_call_render_pass_filter_label, 3, 1, Qt::AlignRight);
    ...
    QGridLayout* render_pass_filter_layout = new QGridLayout(m_what_if_render_pass_filters_container);
    ...
    QStandardItem* what_if_render_pass_command_buffer_filter_placeholder = new QStandardItem("Select a command buffer");
    what_if_render_pass_command_buffer_filter_placeholder->setFlags(what_if_render_pass_command_buffer_filter_placeholder->flags() & ~Qt::ItemIsSelectable);
    m_what_if_render_pass_command_buffer_filter_model->appendRow(what_if_render_pass_command_buffer_filter_placeholder);
    m_what_if_render_pass_command_buffer_filter_box->setModel(m_what_if_render_pass_command_buffer_filter_model);
    render_pass_filter_layout->addWidget(m_what_if_render_pass_command_buffer_filter_label, 0, 1, Qt::AlignRight);
    ...
    QStandardItem* what_if_render_pass_type_filter_placeholder = new QStandardItem("Select render pass type");
    what_if_render_pass_type_filter_placeholder->setFlags(what_if_render_pass_type_filter_placeholder->flags() & ~Qt::ItemIsSelectable);
    m_what_if_render_pass_render_pass_type_filter_model->appendRow(what_if_render_pass_type_filter_placeholder);
    ...
    m_what_if_render_pass_render_pass_type_filter_box->setModel(m_what_if_render_pass_render_pass_type_filter_model);
    render_pass_filter_layout->addWidget(m_what_if_render_pass_render_pass_type_filter_label, 1, 1, Qt::AlignRight);
    ...
    m_what_if_modification_warning_label = new QLabel(tr(
        "⚠ When testing this modification, the application will be relaunched."));
    ...
    QObject::connect(m_what_if_command_box, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &WhatIfConfigureDialog::OnWhatIfModificationCommandChanged);

    // Connections for buttons
    QObject::connect(m_dismiss_button, &QPushButton::clicked, this, &WhatIfConfigureDialog::ResetDialog);
    ...
    QObject::connect(m_what_if_draw_call_index_count_filter_box, QOverload<int>::of(&QSpinBox::valueChanged), this, &WhatIfConfigureDialog::OnUpdateAddModificationButtonState);

    QObject::connect(m_what_if_draw_call_vertex_count_filter_box, QOverload<int>::of(&QSpinBox::valueChanged), this, &WhatIfConfigureDialog::OnUpdateAddModificationButtonState);

    QObject::connect(m_what_if_draw_call_instance_count_filter_box, QOverload<int>::of(&QSpinBox::valueChanged), this, &WhatIfConfigureDialog::OnUpdateAddModificationButtonState);

    QObject::connect(m_what_if_draw_call_draw_count_filter_box, QOverload<int>::of(&QSpinBox::valueChanged), this, &WhatIfConfigureDialog::OnUpdateAddModificationButtonState);

    QObject::connect(m_what_if_draw_call_pso_property_filter_box, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &WhatIfConfigureDialog::OnUpdateAddModificationButtonState);

    QObject::connect(m_what_if_draw_call_render_pass_filter_box, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &WhatIfConfigureDialog::OnUpdateAddModificationButtonState);

    QObject::connect(m_what_if_render_pass_command_buffer_filter_box, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &WhatIfConfigureDialog::OnUpdateAddModificationButtonState);

    QObject::connect(m_what_if_render_pass_render_pass_type_filter_box, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &WhatIfConfigureDialog::OnUpdateAddModificationButtonState);

    // Connect flag model dataChanged for check state changes
    QObject::connect(m_what_if_flag_model, &QStandardItemModel::dataChanged,
            this, &WhatIfConfigureDialog::OnFlagModelChanged);
}
...
{
    const int modification_type_index = m_what_if_type_box->currentIndex() - 1; // -1 because of the placeholder
    ...
    ShowMessage(QString::fromStdString(absl::StrCat("\"", modification_type_info.ui_name_short, "\"", " modification added successfully! View pending modifications in the 'What-If Modifications' tab.")));
}

//--------------------------------------------------------------------------------------------------
void WhatIfConfigureDialog::OnFlagModelChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void WhatIfConfigureDialog::OnUpdateAddModificationButtonState()
{
    ...
    // Based on the modification type, check if the required fields are filled. At least one filter or flag should be selected.
    const auto& modification_type_info = Dive::kWhatIfModificationTypeInfos[modification_type_index];
    ...
    else if (modification_type_info.type == Dive::WhatModificationType::kRenderPassLoadStoreOpOverridden || modification_type_info.type == Dive::WhatModificationType::kRenderPassScissorOverridden)
    {
...
void WhatIfConfigureDialog::OnWhatIfModificationTypeChanged(int index)
{
    ...
    const auto& ty_info = Dive::kWhatIfModificationTypeInfos[index - 1]; // -1 because of the placeholder
    m_what_if_command_box->clear();
