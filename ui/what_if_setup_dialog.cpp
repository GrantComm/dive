...
//--------------------------------------------------------------------------------------------------
bool WhatIfAppTypeFilterModel::filterAcceptsRow(int sourceRow, const QModelIndex& sourceParent) const
{
    if (m_filter_active)
    {
    ...
    : DeviceDialog(parent), m_controller(controller)
{
    ...
    m_what_if_info_label = new QLabel(tr("Modify vulkan calls, shaders, etc. to explore the potential impact rendering the application."));
    ...
    m_runtime_what_if_type_label = new QLabel(tr("This provides fewer options, but allows the running application to be controlled interactively to get an overall feeling for performance."));
    ...
    m_replay_what_if_type_label = new QLabel(tr("This provides more options, but analyzes the effects on a single frame capture."));
    ...
    QObject::connect(m_args_input_box, &QLineEdit::textEdited, this, &WhatIfSetupDialog::OnInputArgs);
    QObject::connect(m_start_application_button, &QPushButton::clicked, this, &WhatIfSetupDialog::OnStartClicked);
...
void WhatIfSetupDialog::OnDeviceSelectionCleared() { m_start_application_button->setEnabled(false); }
...
bool WhatIfSetupDialog::StartPackage(Dive::AndroidDevice* device, const std::string& app_type)
{
    ...
    absl::Status ret;
    qDebug() << "Start app on dev: " << m_cur_device.c_str() << ", package: " << m_cur_pkg << ", type: " << app_type.c_str() << ", args: " << m_command_args.c_str();
    ...
    {
        ret = device->SetupApp(m_cur_pkg.toStdString(), Dive::ApplicationType::OPENXR_APK, m_command_args,
                               /*gfxr_capture_directory*/ "");
    }
    else if (app_type == Dive::kAppTypeInfos[static_cast<size_t>(Dive::AppType::kVulkan_Non_OpenXR)]
                             .ui_name.data())
    {
        ret = device->SetupApp(m_cur_pkg.toStdString(), Dive::ApplicationType::VULKAN_APK, m_command_args,
                               /*gfxr_capture_directory*/ "");
    }
    else if (app_type ==
                 ...
                 .ui_name.data())
    {
        ...
        ret = device->SetupApp(m_cur_pkg.toStdString(), m_command_args, Dive::ApplicationType::VULKAN_CLI,
                               /*gfxr_capture_directory*/ "");
    }
    if (!ret.ok())
    {
        std::string err_msg =
            absl::StrCat("Fail to setup for package ", m_cur_pkg.toStdString(), " error: ", ret.message());
        qDebug() << err_msg.c_str();
        ShowMessage(QString::fromStdString(err_msg));
        return false;
    }
    ...
    {
        std::string err_msg =
            absl::StrCat("Fail to start package ", m_cur_pkg.toStdString(), " error: ", ret.message());
        qDebug() << err_msg.c_str();
        ShowMessage(QString::fromStdString(err_msg));
        return false;
    }
    ...
    {
        std::string err_msg =
            absl::StrCat("Process for package ", m_cur_pkg.toStdString(), " not found, possibly crashed.");
        qDebug() << err_msg.c_str();
        ShowMessage(QString::fromStdString(err_msg));
        return false;
    }
...
void WhatIfSetupDialog::StopPackage()
{
    ...
    if (!status.ok())
    {
        ...
        ShowMessage(QString::fromStdString(absl::StrCat("Failed to cleanup application: ", status.message())));
...
void WhatIfSetupDialog::OnStopRuntimeWhatIf()
{
    StopPackage();
}
...
void WhatIfSetupDialog::OnStartClicked()
{
    ...
    {
        qDebug() << "Start Application clicked with package: " << m_cur_pkg << ", type: " << ty_str.c_str();
...
void WhatIfSetupDialog::closeEvent(QCloseEvent* event)
{
    ...
    bool no_application_currently_running_on_device = (device == nullptr) || (cur_app == nullptr) || !cur_app->IsRunning();
